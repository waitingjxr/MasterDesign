...
export default function SpaceSurface() {
  ...
  const handleQuery = () => {
    axios({
      ...
      x_min = res.data[0]['x_min']
      x_max = res.data[0]['x_max']
      y_min = res.data[0]['y_min']
      y_max = res.data[0]['y_max']
      z_min = res.data[0]['z_min']
      z_max = res.data[0]['z_max']
      var chart_data = res.data.map((item, i) => {
        return [
          item['y'],
          item['x'],
          item['d3']
      ...
      chart_data.push([res.data['y'], res.data['x'], res.data['d3']])
